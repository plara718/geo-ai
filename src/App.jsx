...
import AdminDashboard from './screens/AdminDashboard'; // ★追加
import SettingsModal from './components/SettingsModal';
import Toast from './components/Toast';

function App() {
  const { user, loading: authLoading, loginAsGuest, logout } = useAuthUser();
  const { activeSession, historyMeta, isDailyLimitReached, loading: sessionLoading } = useStudySession(user?.uid);

  // App State
  // currentScreen: 'start' | 'lesson' | 'review' | 'log' | 'vocab' | 'admin'
  ...
  useEffect(() => {
    if (user) {
      getDoc(doc(db, 'artifacts', APP_ID, 'users', user.uid, 'settings', 'config'))
        .then(snap => {
      ...
      <Box sx={{ minHeight: '100vh', bgcolor: '#f8fafc' }}>
        ...
        {currentScreen === 'start' && (
          <StartScreen 
            ...
            // レッスン開始
            onStartLesson={() => {
              if (!apiKey) { setToastMsg("APIキーを設定してください"); setIsSettingsOpen(true); return; }
        ...
        <SettingsModal 
          ...
          onSave={setApiKey}
          onEnterAdmin={() => setCurrentScreen('admin')} // ★遷移関数を渡す
        />
